return (function(...)
	...
	local function Deserialize()
		local y, z = buffer.fromstring(w:gsub("..", function(y)
		...
		local function A()
			...
			F[126] = buffer.readi32(y, z);
			z = z + 4;
			while true do
				local G = buffer.readu8(y, z);
				z = z + 1;
				if G ~= 0 then
					E = E .. string.char(G);
				else
			...
			F[7] = G;
			z = z + 1;
			local H = buffer.readu32(y, z);
			z = z + 4;
			for I = 0, H - 1 do
				local J = buffer.readu8(y, z);
				z = z + 1;
				if J ~= 0 then
					if J ~= 1 then
						if J == 2 then
							local K = buffer.readu32(y, z);
							z = z + 4;
							B[I] = buffer.readstring(y, z, K);
							z = z + K;
						else
							B[I] = {};
						end;
					else
						B[I] = buffer.readf64(y, z);
						z = z + 8;
					end;
				else
					B[I] = nil;
					...
					if M ~= 27 then
						if M ~= 21 then
							if M ~= 74 then
													...
													if M ~= 65 then
														if M ~= 4 then
															if M ~= 73 then
																		...
																		if M ~= 39 then
																			if M ~= 5 then
																				if M ~= 12 then
																					...
																					if Q > 1 then
																						local R = B[C[bit32.band(bit32.rshift(O, 20), 1023)]];
																						local S = R[C[bit32.band(bit32.rshift(O, 10), 1023)]];
																						if Q > 2 then
																							D[N] = S[C[bit32.band(O, 1023)]];
																						else
																							D[N] = S;
																						end;
																					else
																...
																local U = table.pack(T(D[P]));
																if S == (-1) then
																	E = R + (#U);
																	S = #U;
																end;
																table.move(U, 1, S, R, D);
																G = G + N + (1);
															end;
														else
															local N, O = L[90], L[4];
								...
								local V = table.pack(T(D[Q], D[N]));
								if S == (-1) then
									E = U + (#V);
									S = #V;
								end;
								table.move(V, 1, S, U, D);
							end;
						else
							...
							local S = table.pack(Q(table.unpack(D, N + (1), R - (1))));
							if P == (-1) then
								E = N + (#S);
								P = #S;
							end;
							table.move(S, 1, P, N, D);
						end;
					else
